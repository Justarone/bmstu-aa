...
fn input_matrix(s1: usize, s2: usize) -> Vec<Vec<MatInner>> {
    ...
    info!("Введите через пробел элементы матрицы размеров {}x{}:", s1, s2);
...
pub fn read_data() -> (Vec<Vec<MatInner>>, Vec<Vec<MatInner>>) {
    info!("Введите n, k, m через пробел (матрицы в таком случае будут n x k, k x m): ");
...
pub fn measure(n: usize, sizes: [usize; 3]) {
    info!("Производится {0} замеров для матриц размера {1} x {2}, {2} x {3}", n, sizes[0], sizes[1], sizes[2]);
